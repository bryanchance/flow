...
import (
	...
	"github.com/ehazlett/flow/cmd/flow-workflow-docker/processor"
...
func workerAction(clix *cli.Context) error {
	...
	dockerAddress := clix.String("docker")
	dockerUsername := clix.String("docker-username")
	dockerPassword := clix.String("docker-password")

	p, err := processor.NewProcessor(cfg, &processor.DockerConfig{
